...
def test_missing_modalities_keep_state_unchanged() -> None:
    scorer = EngagementScorer()
    out = scorer.update(EngagementInputs(emotion_label=None))
...
def test_intermediate_score_yields_engaging() -> None:
    scorer = EngagementScorer()
    # Use a less-weighted emotion (sad = 0.3/0.9 ≈ 0.33 max).
    inputs = EngagementInputs(
        emotion_label="sad", yaw_deg=0.0, pitch_deg=0.0
    )
    final = None
    for _ in range(40):
...
def test_pose_gate_demotes_engaged_when_yaw_too_large() -> None:
    ...
    pose_off = EngagementInputs(
        emotion_label="neutral", yaw_deg=45.0, pitch_deg=0.0
    )
    final = None
    for _ in range(40):
...
def test_low_weight_emotion_stays_disengaged() -> None:
    scorer = EngagementScorer()
    inputs = EngagementInputs(
        emotion_label="disgust", yaw_deg=0.0, pitch_deg=0.0
    )
    final = None
    for _ in range(40):
        final = scorer.update(inputs)
